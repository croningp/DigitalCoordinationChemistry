...
def alpine1(xi:list):
    ...
    for x in xi:
        f1 = abs(x) * np.sin(abs(x)) + (0.1 * abs(x))
        f2 += f1
    return f2
...
def rastrigin(xi:list, a:float=10, b:float=2):
    # Rastrigin Function
    # n-dimensional
    highD.rastrigin.dimensions = len(xi)
    sum_3 = 0 #np.empty(x1.shape)
    for item in xi:
        sum_1 = a * np.cos(b * np.pi * item)
        sum_2 = item**2 - sum_1
        sum_3 += sum_2
    f_1 = 10 * highD.rastrigin.dimensions + sum_3
    return f_1
